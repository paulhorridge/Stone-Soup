...
if ~isfield(params, 'trackConfirmLogProbThresh')
    params.trackConfirmLogProbThresh = -inf;
end
...
for measNum = 1:numScans
    ...
    % (change later?)
    if max(hypLogWeights)==hypLogWeights(end)
        if hypLogWeights(end) > params.trackConfirmLogProbThresh
